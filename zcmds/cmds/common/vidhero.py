...
def main():
    ...
    parser.add_argument("--state_timecode", help="start timecode like 0:38", required=True)
    ...
    start_timecode = args.state_timecode or input("Enter start timecode like 0:38: ")
    ...
    for height in args.heights:
        ...
        out_path = os.path.join(path, f"{os.path.basename(path)}_hero.mp4")
        os.makedirs(os.path.dirname(out_path), exist_ok=True)
        # trunc(oh*...) fixes issue with libx264 encoder not liking an add number of width pixels.
        cmd = f'static_ffmpeg -hide_banner -i "{video_path}" -ss {start_timecode} -to {end_timecode} -vf "scale=trunc(oh*a/2)*2:{height},fade=t=in:st=124:d=1,fade=t=out:st=187:d=1" -movflags +faststart -preset veryslow -c:v libx264 -crf {crf} "{out_path}"'
        print(f"Running:\n  {cmd}")
        os.system(cmd)
