...
def gen_macos_cmds():
    common_cmds = [os.path.abspath(os.path.join(COMMON_DIR, f)) for f in os.listdir(COMMON_DIR)]
    macos_cmds = [os.path.abspath(os.path.join(SELF_DIR, f)) for f in os.listdir(SELF_DIR)]
    all_cmds = common_cmds + macos_cmds
    all_cmds = [cmd for cmd in all_cmds if not os.path.basename(cmd).startswith('_') and cmd.endswith(".py")]
...
def add_cmds_to_path():
    ...
    #    print("")
    if last_path_line == -1:
        raise ValueError(f"Could not find a place to splice in {OUT_CMD_DIR} into {bash_profile_file}, please do it manually.")
